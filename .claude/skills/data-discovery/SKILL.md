...
- The output database contains:
  ...
  - `behavior_pnr` (booking-level data)
  - `behavior_pnr_leg` (flight leg-level data)
  - `behavior_pnr_leg_anc` (ancillary purchases)
...
tdx query "SHOW COLUMNS FROM behavior_pnr_leg_anc" --parent-segment cdp_lhg_unified_first_party
...
tdx query "SELECT COUNT(*) FROM behavior_pnr WHERE ..."

# CORRECT — always qualify:
tdx query "SHOW COLUMNS FROM cdp_audience_1159510.customers"
tdx query "SELECT COUNT(*) FROM cdp_audience_1159510.behavior_pnr WHERE ..."
```
...
| behavior_pnr_leg | Flight legs | oper_comp_cd, mkt_comp_cd, oper_airl_cd, mkt_airl_cd, dest_cntry_cd, traffic_typ, tvl_txn_id |
| behavior_pnr_leg_anc | Ancillaries | rev_type_lvl_2_code, rev_type_lvl_5_code, res_status_categ_cd, coup_status_cd, emd_coup_status_cd |

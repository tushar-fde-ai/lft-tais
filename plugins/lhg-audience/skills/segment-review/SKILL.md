---
name: segment-review
description: Review and validate LHG audience segmentation logic against the Lufthansa Group segmentation policy. Use when checking segment compliance, reviewing segment rules, or when the run skill needs pre-creation validation. Triggers on: "review segment", "check segmentation", "validate audience", "policy check", "review rules".
---
...
4. Express every recency/date window in DAYS, never in "months" or "years".
5. When targeting by airline, combine operating_airline OR marketing_airline.
6. When targeting by cabin class, combine the columns oper_comp_cd OR mkt_comp_cd.
...
9. Keep each web bucket's attributes together in its own container; combine the brand-family buckets with ANY (OR).
...
## 3. AUDIENCE STUDIO — BUILDER RULES [MANDATORY]

### 3.1 Filters — container structure

- Always create ALL audience filters within the SAME container.
- Exception: when intentionally wanting OR between groups, put each group in its own container and combine with ANY.
...
- Keep all attributes of the SAME bucket together in that bucket's container.
...
- When multiple containers are used, verify the combination logic (ANY/ALL) matches the intended boolean relationship.
